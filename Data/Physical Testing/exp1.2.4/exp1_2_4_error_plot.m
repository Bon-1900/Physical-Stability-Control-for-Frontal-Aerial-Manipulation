...
for i_bag_paths = 1:length(bag_paths)
    ...
    % data correction due to arena setup
    if ismember(i_bag_paths, 3:4)
    data_mat(:,2) = data_mat(:,2) - 3.644;
    data_mat(:,3) = data_mat(:,3) + 4.164;
    sp_mat = sp_mats{2};
    sp_mat = remap_sp(sp_mat(:,[2,3,4]), sp_mat(:,1),data_mat(:,1));
    end 
...
xlabel('Simulation Time (sec)')
ylabel('Error (m)')
title ('Exp1.2.4 Absolute Translational error')
